...
import { GrafanaIcon } from "@/components/shared/grafana-icon";
      ...
      <header className={cn("sticky top-0 z-40 w-full bg-background border-b border-border")}>
        <div className={cn("max-w-7xl mx-auto px-4 h-20 flex items-center justify-between gap-8")}>
          ...
          <div className={cn("flex items-center gap-1 sm:gap-2")}>
            ...
            {/* Grafana Mobile Button */}
            <a
              href={grafanaUrl}
              target="_blank"
              rel="noopener noreferrer"
              className={cn(
                "md:hidden flex items-center justify-center size-9 rounded-full text-[#F46800] hover:bg-orange-50 dark:hover:bg-orange-950/40 transition-colors"
              )}
              ...
              >
                <form onSubmit={handleSearchSubmit} className="flex items-center gap-2">
                  ...
                  <Button
                    type="submit"
                    size="sm"
                    className="h-9 px-3 rounded-xl bg-emerald-950 hover:bg-emerald-900 text-white text-xs font-semibold cursor-pointer"
                  >
                    Search
                  </Button>
                </form>
            ...
            >
              ...
              {wishlistCount > 0 && (
                <span
                  className={cn(
                    "absolute top-0.5 right-0.5 min-w-4 h-4 px-1 rounded-full bg-emerald-800 text-white text-[10px] font-semibold flex items-center justify-center leading-none"
                  )}
                >
                  {wishlistCount}
                </span>
            ...
            {!isLoading && user ? (
              <Popover>
                <PopoverTrigger
                  className={"size-9 rounded-full bg-emerald-900 text-emerald-100 font-bold text-sm flex items-center justify-center shrink-0 shadow-inner cursor-pointer"}
